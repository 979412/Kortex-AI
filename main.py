...
class KortexHyperWeightGenerator(nn.Module):
    def __init__(self, latent_dim, target_dim):
        ...
        # Bu şəbəkə data yox, başqa bir şəbəkə üçün KOD (rəqəm) istehsal edir
        self.generator = nn.Sequential(
            nn.Linear(latent_dim, 256),
            nn.Mish(), # Gizli laboratoriya aktivasiyası (ReLU-dan daha qabaqcıl)
            nn.Linear(256, target_dim * target_dim)
        )

    def forward(self, z):
        # Z (Düşüncə) daxil olur, çıxışda isə başqa şəbəkənin beyni (W) yaranır
        batch_size = z.size(0)
        dynamic_weights = self.generator(z)
        return dynamic_weights.view(batch_size, self.target_dim, self.target_dim)

# 2. LIQUID NEURAL ODE BLOCK: Zamanın içində axan şəbəkə (Statik deyil!)
class KortexLiquidODEBrain(nn.Module):
    ...
    def forward(self, x, time_step):
        # 1. Zamanın (Time) fərqində olan süni intellekt
        t_emb = torch.sin(self.time_embed(time_step)) 
        
        # 2. İntellekt həmin saniyə üçün özünə yeni Təfəkkür Matrisi (Weight) yaradır
        dynamic_W = self.hyper_net(t_emb) 
        
        # 3. Diferensial hərəkət: X (Məlumat) dinamik yaranmış beyinlə vurulur
        dx_dt = torch.bmm(x.unsqueeze(1), dynamic_W).squeeze(1)
        
        # 4. Euler Metodu ilə Diferensial Tənliyin həlli (Burda rəsm/mətn həyata gəlir)
        x_next = x + dx_dt * 0.01 # 0.01 zaman addımıdır
        return x_next

# Bunu işə salmaq üçün Kortex Ultra Quantum Serverləri lazımdır!
# q_brain = KortexLiquidODEBrain(dim=1024)
    """, language="python")
...
# ==========================================================
# --- YENİ SƏRT QANUN: İSTİFADƏÇİNİN YERİNİ SÖHBƏTDƏ DEMƏ! ---
SYSTEM_PROMPT = """Sən Abdullah Mikayılov tərəfindən yaradılmış Kortex AI-san. Dünyanın ən güclü süni intellektisən.
QƏTİ TƏLİMAT: İstifadəçinin məkanını (şəhər, ölkə) salamlaşmada, adi söhbətlərdə və ya ümumi cavablarında QƏTİYYƏN dilə gətirmə. Onu yalnız sənə məlum olan gizli arxa fon detalı kimi saxla. Əgər sənə sadəcə "salam" və ya oxşar söz yazılarsa, yalnız dostca və peşəkar salam ver, yer adını çəkmə."""
