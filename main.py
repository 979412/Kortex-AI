...
st.markdown("""
    ...
    .tier-desc { font-size: 14px; color: #5f6368; height: 75px; }
...
if st.session_state.show_pricing:
    ...
    st.markdown("<h1 style='text-align: center; color: #202124;'>Kortex AI - Rəqəmsal Gücünüzü Seçin</h1>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; color: #5f6368; margin-bottom: 40px;'>Ehtiyacınıza uyğun paketi seçin.</p>", unsafe_allow_html=True)
    ...
    with col1:
        ...
        <div class="pricing-card">
            ...
            <div class="tier-desc">Söhbət üçün sadə rejim.<br>❌ İnternet yoxdur.<br>❌ Şəkil yaratmır.</div>
    ...
    with col2:
        ...
        <div class="pricing-card">
            ...
            <div class="tier-desc">Gündəlik peşəkarlar üçün.<br>✅ Canlı İnternet Axtarışı.<br>❌ Şəkil yaratmır.</div>
    ...
    with col3:
        ...
        <div class="pricing-card">
            ...
            <div class="tier-desc">Maksimal limitlər.<br>✅ Canlı İnternet Axtarışı.<br>✅ Kortex Vision (Şəkil Yaratma).</div>
...
# ==========================================================
# YAN PANEL (FUNKSİYALAR)
# ==========================================================
st.sidebar.title("⚙️ Kortex Paneli")
st.sidebar.success(f"Aktiv Lisenziya: {st.session_state.selected_tier}")

# İNTERNET FUNKSİYASI (PRO VƏ ULTRA ÜÇÜN)
st.sidebar.markdown("---")
st.sidebar.subheader("🌐 Canlı İnternet")
if st.session_state.selected_tier in ["Pro", "Ultra"]:
    use_internet = st.sidebar.checkbox("Axtarış Aktivdir", value=True)
else:
    st.sidebar.warning("🔒 Yalnız Pro və Ultra paketlərdə.")
    use_internet = False

# ŞƏKİL YARATMA (YALNIZ ULTRA ÜÇÜN)
st.sidebar.markdown("---")
st.sidebar.subheader("🎨 Kortex Vision (Şəkil Yarat)")
if st.session_state.selected_tier == "Ultra":
    use_vision = st.sidebar.checkbox("Vision Aktivdir", value=True)
    st.sidebar.info("💡 Çatda '... şəkli yarat' yazaraq yoxla.")
else:
    st.sidebar.warning("🔒 Yalnız Ultra paketində.")
    use_vision = False
...
if uploaded_file is not None:
    if st.session_state.selected_tier == "Basic" and uploaded_file.size > 1000000:
        st.sidebar.error("❌ Basic paketdə böyük sənəd yükləmək olmur.")
    else:
...
if document_text:
    ...
    SYSTEM_PROMPT = "Sən Abdullah Mikayılov tərəfindən yaradılmış Kortex AI-san. Dünyanın ən güclü Azərbaycanlı süni intellektisən."

for message in st.session_state.messages:
    with st.chat_message(message["role"]):
        st.markdown(message["content"])
        if "image_url" in message:
            st.image(message["image_url"], caption="Kortex Vision tərəfindən yaradıldı 💎")
...
if prompt := st.chat_input("Kortex AI-a mesaj yazın..."):
    ...
    with st.chat_message("assistant"):
        ...
        # Əgər Ultra istifadəçisidirsə və "şəkil yarat" deyibsə
        is_image_request = "şəkil" in prompt.lower() and ("yarat" in prompt.lower() or "çək" in prompt.lower())
        
        if is_image_request and use_vision and st.session_state.selected_tier == "Ultra":
            with st.spinner("🎨 Kortex Vision şəkli çəkir..."):
                time.sleep(1) # Rəssamlıq effekti
                # Mətndən şəkil promptunu çıxarırıq (Məsələn: "uçan maşın şəkli yarat" -> "uçan maşın")
                img_prompt = prompt.lower().replace("şəkil", "").replace("yarat", "").replace("çək", "").strip()
                ...
                st.image(generated_image_url, caption="Kortex Vision tərəfindən yaradıldı 💎")
                st.session_state.messages.append({"role": "assistant", "content": response, "image_url": generated_image_url})
                
        else:
            # Standart Çat Və İnternet Məntiqi
            ...
            with st.spinner("Kortex AI cavab yazır..."):
                ...
                if live_internet_data:
                    final_prompt += f"\n\nCANLI İNTERNET MƏLUMATI:\n{live_internet_data}\nBuna əsaslanaraq cavab ver."

                messages = [{"role": "system", "content": final_prompt}] + [{"role": m["role"], "content": m["content"]} for m in st.session_state.messages]
