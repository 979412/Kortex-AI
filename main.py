...
# ==========================================================
# QİYMƏT EKRANI VƏ ÖDƏNİŞ (İxtisar edilmişdir, amma eynidir)
# ==========================================================
if st.session_state.show_pricing:
    ...
    with col1:
        st.markdown("""<div class="pricing-card"><div class="tier-name">Kortex Basic</div><div class="tier-price">$0</div><div class="tier-desc"><ul><li>Mətn: Llama 3 8B</li></ul></div></div>""", unsafe_allow_html=True)
        if st.button("Basic Seç", key="btn_b"):
            st.session_state.selected_tier = "Basic"; st.session_state.show_pricing = False; st.rerun()
    with col2:
        st.markdown("""<div class="pricing-card"><div class="tier-name">Kortex Pro</div><div class="tier-price">$12</div><div class="tier-desc"><ul><li>Mətn: Llama 3.3 70B</li></ul></div></div>""", unsafe_allow_html=True)
        if st.button("Pro Əldə Et", key="btn_p"):
            st.session_state.selected_tier = "Pro"; st.session_state.payment_successful = False; st.session_state.show_pricing = False; st.rerun()
    with col3:
        st.markdown("""<div class="pricing-card" style="border-color: #1a73e8;"><div class="tier-name">Kortex Ultra 💎</div><div class="tier-price">$95</div><div class="tier-desc"><ul><li>Mətn: Mixtral 8x7B</li></ul></div></div>""", unsafe_allow_html=True)
        if st.button("Ultra Əldə Et", key="btn_u"):
            st.session_state.selected_tier = "Ultra"; st.session_state.payment_successful = False; st.session_state.show_pricing = False; st.rerun()
    st.stop()
    ...
    with col_pay:
        ...
        if st.button("Aktivləşdir", use_container_width=True):
            with st.spinner("💳 Bankla əlaqə yaradılır..."):
                time.sleep(1); st.session_state.payment_successful = True; st.rerun()
        if st.button("Ləğv Et", use_container_width=True):
            st.session_state.selected_tier = "Basic"; st.session_state.payment_successful = True; st.rerun()
...
if prompt := st.chat_input(f"Kortex AI ({st.session_state.selected_tier} Mode) əmrini gözləyir..."):
    ...
    with st.chat_message("assistant"):
        ...
        if use_internet:
            with st.spinner(f"🌐 Deep Research axtarır..."):
                try:
        ...
        with st.spinner(f"Kortex AI ({active_llm_model}) düşünür..."):
            ...
            try:
                if client:
                    ...
                    response = "XƏTA: Groq obyekti (client) yaradılmayıb. Şifrə düzgün daxil edilməyib."
                    
            except Exception as e:
                # BAX BURADA BÜTÜN GİZLƏDİLMİŞ XƏTALARI EKRANA ÇIXARTDIM:
                response = f"🚨 **GROQ SERVERİNDƏN GƏLƏN RƏSMİ XƏTA:**\n\n`{str(e)}`\n\n*Qardaşım, əgər yuxarıda '401' və ya 'Invalid API Key' yazısı görürsənsə, deməli problem kodda deyil, Groq sənin şifrəni (bura atdığın üçün) ləğv edib. Tətbiqini satmaq üçün məcbur yenisini alıb kodun 70-ci sətrinə heç kimə göstərmədən yapışdırmalısan.*"
