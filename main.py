...
# ==========================================================
# KORTEX CORE: DÜNYANIN ƏN GÜCLÜ DiT ARXİTEKTURASI (YENİ)
# ==========================================================
with st.sidebar.expander("💻 Kortex Core: Ultra Neural Network (Beast Mode)", expanded=False):
    st.markdown("""
    **Mürəkkəb Sİ Arxitekturası:**
    Kortex Vision arxa planda ən yeni nəsil DiT (Diffusion Transformer) və Multi-Head Attention texnologiyasından istifadə edərək piksellər arasında riyazi qüsursuzluq yaradır.
    """)
    st.code("""
import torch
import torch.nn as nn

# 1. DÜNYANIN ƏN GÜCLÜ DİQQƏT MƏRKƏZİ (Multi-Head Self Attention)
class AttentionBlock(nn.Module):
    def __init__(self, dim, heads=16):
        super().__init__()
        self.heads = heads
        self.scale = (dim // heads) ** -0.5
        # Q (Sorğu), K (Açar), V (Dəyər) - Piksellər bir-birini buradan tanıyır
    ...
    def forward(self, x):
        ...
        # Piksellərin bir-biri ilə riyazi əlaqəsi (Sənin istədiyin Qüsursuz Həndəsə buradan yaranır)
...
# 2. DiT (DIFFUSION TRANSFORMER) CANAVAR BLOKU
class DiTBlock(nn.Module):
    def __init__(self, hidden_size):
        ...
        # Mürəkkəb Həndəsə üçün FeedForward şəbəkəsi
    ...
    def forward(self, x, condition_embedding):
        # Mətndəki əmrin (məsələn: qara BMW) piksellərə yeridilməsi
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(condition_embedding).chunk(6, dim=1)
        
        # Diqqət mexanizminin işə düşməsi
        x = x + gate_msa.unsqueeze(1) * self.attn(self.norm1(x) * (1 + scale_msa.unsqueeze(1)) + shift_msa.unsqueeze(1))
        
        # Detalların (boya parıltısı, işıq) işlənməsi
        x = x + gate_mlp.unsqueeze(1) * self.mlp(self.norm2(x) * (1 + scale_mlp.unsqueeze(1)) + shift_mlp.unsqueeze(1))
        return x

# Bu kodu işə salmaq üçün 10.000 ədəd H100 GPU klasteri lazımdır!
    """, language="python")
